...
namespace AuthService.Application.Tests.Features.Users.DeactivateUser;
...
public class UserDeactivatedDomainEventHandlerTests
{
    ...
    public async Task Handle_WithValidDomainEvent_ShouldPublishIntegrationEventOnce()
    {
        ...
        UserDeactivatedIntegrationEvent? publishedEvent = null;

        _integrationEventPublisherMock
            .Setup(publisher => publisher.PublishAsync(It.IsAny<UserDeactivatedIntegrationEvent>(), cancellationToken))
            .Callback<UserDeactivatedIntegrationEvent, CancellationToken>((integrationEvent, _) => publishedEvent = integrationEvent)
        ...
        _integrationEventPublisherMock.Verify(
            publisher => publisher.PublishAsync(It.IsAny<UserDeactivatedIntegrationEvent>(), cancellationToken),
            Times.Once);
