...
public class UserActivatedDomainEventHandlerTests
{
    ...
    public async Task Handle_WithValidDomainEvent_ShouldEnqueueIntegrationEventOnce()
    {
        ...
        UserActivatedIntegrationEvent? enqueuedEvent = null;

        _outboxWriterMock
            .Setup(writer => writer.EnqueueAsync(It.IsAny<UserActivatedIntegrationEvent>(), cancellationToken))
            .Callback<UserActivatedIntegrationEvent, CancellationToken>((integrationEvent, _) => enqueuedEvent = integrationEvent)
        ...
        _outboxWriterMock.Verify(
            writer => writer.EnqueueAsync(It.IsAny<UserActivatedIntegrationEvent>(), cancellationToken),
            Times.Once);
